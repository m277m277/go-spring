...
// with the SQL Server db.system semantic-convention value. The plugin
// implementation is shared by every gorm starter; only this label differs.
func applyObservability(db *gorm.DB, c Config) error {
	return db.Use(gormobserve.NewPlugin("microsoft.sql_server", c.Observability))
}
