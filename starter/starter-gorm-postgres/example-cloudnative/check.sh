#!/usr/bin/env bash
#
# Smoke test for the starter-gorm-postgres cloud-native flagship. Brings up a
# local PostgreSQL via docker compose, runs the example (which self-asserts
...
for _ in $(seq 1 45); do
    status="$(docker inspect -f '{{.State.Health.Status}}' starter-gorm-postgres-cloudnative 2>/dev/null || true)"
