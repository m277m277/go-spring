# starter-gorm-postgres Health Example

Demonstrates starter-gorm-postgres's **health integration**: each configured
...
Requires a local PostgreSQL (`docker compose up -d`):

```bash
cd starter/starter-gorm-postgres/example-health
docker compose up -d
go run . -manual
```
...
`check.sh` brings up PostgreSQL via docker compose, then asserts the DB pings
and the actuator probes report UP; exit code 0 means pass. Skipped gracefully
